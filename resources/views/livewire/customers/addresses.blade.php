<div>
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
        @forelse($addresses as $address)
            <div class="relative flex items-center px-6 py-5 rounded-lg bg-white shadow dark:bg-gray-800">
        ...
        @empty
            <div class="sm:col-span-3 relative rounded-lg bg-white px-6 py-10 sm:py-12 shadow flex flex-col items-center">
                <div class="flex-shrink-0 h-24 w-24 mx-auto text-gray-400">
                    <x-heroicon-o-map class="w-full h-full" />
                </div>
                <div class="mt-5 w-full sm:max-w-md space-y-2 text-center">
                    <p class="text-base leading-6 text-gray-900 font-medium dark:text-white">{{ __('Customer addresses') }}</p>
                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('This customer does not yet have a delivery or billing address.') }}</p>
                </div>
            </div>
        @endforelse
    </div>
