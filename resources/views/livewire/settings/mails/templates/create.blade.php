...
<div>
    ...
    <div class="mt-3 pb-5 border-b border-gray-200">
        <h3 class="text-2xl font-bold leading-6 text-gray-900 sm:text-3xl sm:leading-9 sm:truncate">
            {{ ucfirst($type) }} / {{ ucfirst($skeleton['name']) }} / {{ ucfirst($skeleton['skeleton']) }}
        </h3>
    </div>
    ...
    <div class="mt-6 pb-10">
        <div class="grid gap-4 sm:grid-cols-4 sm:gap-6">
            <div class="sm:col-span-3" wire:ignore>
                <textarea id="template_editor" wire:model.debounce.300ms="template" cols="30" rows="10">{{ $template }}</textarea>
            </div>
            <div class="sm:col-span-1 space-y-5">
                <x-shopper-input.group for="name" label="Template name" :error="$errors->first('name')" isRequired>
                    <x-shopper-input.text wire:model="name" id="name" placeholder="Shopper Newsletter" />
                </x-shopper-input.group>
                <x-shopper-input.group for="description" label="Template description" :error="$errors->first('description')" isRequired>
