<x-livewire-tables::table.cell>
    <div class="flex items-center space-x-3 lg:pl-2">
        <div class="flex-shrink-0 w-2.5 h-2.5 rounded-full {{ $row->approved ? 'bg-green-600': 'bg-gray-200' }}"></div>
        <a href="{{ route('shopper.reviews.show', $row) }}" class="flex items-center">
            ...
            <span class="ml-2 truncate flex flex-col">
                ...
                <span class="text-xs leading-4 text-gray-500 dark:text-gray-400">{{ $row->reviewrateable->formatted_price ?? __('N/A') }}</span>
            </span>
        </a>
    </div>
</x-livewire-tables::table.cell>

<x-livewire-tables::table.cell class="table-cell whitespace-no-wrap text-sm leading-5 text-gray-500 dark:text-gray-400">
    <div class="flex items-center">
        <div class="flex-shrink-0 h-8 w-8">
            <img class="h-8 w-8 rounded-full" src="{{ $row->author->picture }}" alt="">
        </div>
        <div class="ml-4 truncate">
            <div class="text-sm leading-5 font-medium text-gray-900 dark:white">{{ $row->author->full_name }}</div>
...
<x-livewire-tables::table.cell class="hidden md:table-cell whitespace-no-wrap text-sm leading-5 text-gray-500 dark:text-gray-400">
    <div>
        ...
        <p class="text-sm text-gray-500 leading-5 dark:text-gray-400">{{ str_limit($row->content, 40) }}</p>
    </div>
</x-livewire-tables::table.cell>
                        ...
                        <div x-data="{ open: false }" x-on:item-removed.window="open = false" @keydown.escape="open = false" @click.away="open = false" class="relative flex justify-end items-center">
                            ...
                            >
                                <div class="relative z-10 rounded-md bg-white shadow-xs dark:bg-gray-800" role="menu" aria-orientation="vertical" aria-labelledby="project-options-menu-0">
                                    <div class="py-1">
                                        <a href="" type="button" class="group w-full flex items-center px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900 dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:bg-gray-700" role="menuitem">
