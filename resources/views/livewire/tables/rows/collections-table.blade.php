...
<x-livewire-tables::table.cell>
    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium leading-4 {{ $row->type === "auto" ? 'bg-green-100 text-green-800' :  'bg-primary-100 text-primary-800' }}">
        <svg class="-ml-1 mr-1.5 h-2 w-2 {{ $row->type === "auto" ? 'text-green-400' :  'text-primary-400' }}" fill="currentColor" viewBox="0 0 8 8">
            <circle cx="4" cy="4" r="3" />
        </svg>
        {{ $row->type === 'auto' ? __('Automatic') : __('Manual') }}
    </span>
</x-livewire-tables::table.cell>
