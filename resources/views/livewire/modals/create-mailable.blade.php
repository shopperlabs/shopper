    ...
    <x-slot name="content">
        <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
            <x-shopper-input.group label="Name" for="name" class="sm:col-span-2" :error="$errors->first('name')" helpText="Enter mailable name e.g Welcome User, WelcomeUser" isRequired>
                <x-shopper-input.text wire:model.defer="name" type="text" id="name" placeholder="Mailable name" />
            </x-shopper-input.group>
            <div class="sm:col-span-2">
                <div class="relative flex items-start">
                    <div class="flex items-center h-5">
                        <x-shopper-input.checkbox wire:model.defer="isMarkdown" id="is_markdown" />
                    </div>
            ...
            @if($isMarkdown)
                <x-shopper-input.group label="Markdown" for="markdown" class="sm:col-span-2" :error="$errors->first('markdownView')" isRequired>
                    <x-shopper-input.text wire:model.defer="markdownView" type="text" id="markdown" placeholder="Eg. markdown.view" />
                </x-shopper-input.group>
            @endif
            ...
            <div class="sm:col-span-2">
                <div class="relative flex items-start">
                    <div class="flex items-center h-5">
                        <x-shopper-input.checkbox wire:model.defer="isForce" id="is_force" />
                    </div>
                    <div class="ml-3 text-sm leading-5">
