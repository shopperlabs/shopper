<div>
            ...
            <div class="px-4 sm:px-5 flex items-center justify-between space-x-4">
                ...
                <div>
                    <span class="shadow-sm rounded-md">
                        <x-shopper-dropdown-split>
                            <x-slot name="button">
                                <x-shopper-button type="button" wire:click="confirmVarianteCreating" class="border-r-none rounded-r-none mr-0">
                                    {{ __('Add variant') }}
                                </x-shopper-button>
                            </x-slot>
                            ...
                            <x-slot name="content">
                                <div class="py-1">
                                    <x-shopper-dropdown-button wire:click="openBulkVariantsModal" role="menuitem">
                                        ...
                                        {{ __('Add variants from attribute matrix') }}
                                ...
                                <tr>
                                    <td class="px-6 py-3 max-w-xl text-sm leading-5 font-medium text-gray-900">
                                        <div class="flex items-center space-x-3 lg:pl-2">
                                            <div class="flex-shrink-0 w-2.5 h-2.5 rounded-full {{ $variant->is_visible ? 'bg-green-600' : 'bg-gray-400' }}">
                                            </div>
                                            <a href="{{ route('shopper.products.edit', $variant->id) }}" class="flex items-center">
                                    ...
                                    <td class="px-6 py-3 table-cell whitespace-no-wrap text-sm leading-5 text-gray-500 font-medium">
                                        <span>
                                            @if ($variant->formattedPrice)
                                            {{ $variant->formattedPrice }} @else &mdash; @endif
                                        </span>
                                    </td>
                                    ...
                                    <td class="pr-6">
                                        <div x-data="{ open: false }" x-on:item-removed.window="open = false" @keydown.escape="open = false" @click.away="open = false" class="relative flex justify-end items-center">
                                            <button id="project-options-menu-0" aria-has-popup="true" :aria-expanded="open" type="button" @click="open = !open"
                                            ...
                                            <div x-show="open" x-description="Dropdown panel, show/hide based on dropdown state." x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95"
                                                ...
                                                x-transition:leave-end="transform opacity-0 scale-95" class="mx-3 origin-top-right absolute right-7 top-0 w-48 mt-1 rounded-md shadow-lg" style="display: none;">
                                                <div class="relative z-10 rounded-md bg-white shadow-xs" role="menu" aria-orientation="vertical" aria-labelledby="project-options-menu-0">
                                                    <div class="py-1">
                                                        <a href="{{ route('shopper.products.edit', $variant->id) }}"
                                                            ...
                                                            role="menuitem">
                                                            <svg class="mr-3 h-5 w-5 text-gray-400 group-hover:text-gray-500 group-focus:text-gray-500" x-description="Heroicon name: pencil-alt" xmlns="http://www.w3.org/2000/svg"
                                                    ...
                                                    <div class="py-1">
                                                        <button wire:click="remove({{ $variant->id }})" type="button"
                                                            class="group flex w-full items-center px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900"
                                                            role="menuitem">
                                                            <svg class="mr-3 h-5 w-5 text-gray-400 group-hover:text-gray-500 group-focus:text-gray-500" x-description="Heroicon name: trash" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                                                                fill="currentColor">
                                                                <path fill-rule="evenodd"
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-3 whitespace-no-wrap text-sm leading-5 font-medium text-gray-900">
                                        <div class="flex justify-center items-center space-x-2">
                                            ...
                                            <span class="font-medium py-8 text-cool-gray-400 text-xl">{{ __('No variants found') }}...</span>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                        ...
                        <div class="flex-grow space-y-1 lg:flex-grow-0 lg:flex-shrink-0">
                            ...
                            <div class="hidden lg:block">
                                <div class="relative rounded-md overflow-hidden transition duration-150 ease-in-out">
                                    ...
                                    @endif
                                    <label
                                        class="absolute inset-0 w-full h-full rounded-md bg-black bg-opacity-75 flex items-center justify-center text-sm leading-5 font-medium text-white opacity-0 hover:opacity-100 focus-within:opacity-100 transition duration-150 ease-in-out">
                                        <span>{{ __('Change') }}</span>
                        ...
                        @if ($inventories->count() > 1)
                            <div class="py-4 sm:py-5">
                                <div class="flex items-center justify-between">
                                    <h4 class="block text-sm font-medium leading-5 text-gray-900">
                                        {{ __('Quantity Inventory') }}</h4>
                                    <a href="{{ route('shopper.settings.inventories.index') }}"
                                        class="text-sm leading-5 bg-transparent outline-none focus:outline-none text-blue-600 hover:text-blue-800 transition duration-150 ease-in-out">{{ __('Manage Inventories') }}</a>
                                </div>
                                <div class="mt-4 divide-y divide-gray-200">
                                    ...
                                    @foreach ($inventories as $inventory)
                                        <div class="grid grid-cols-3 py-4" wire:key="inventory-{{ $inventory->id }}">
                                            ...
                                            <div class="col-span-1 pl-4 flex justify-end">
                                                <input wire:model="quantity.{{ $inventory->id }}" type="number" class="form-input block w-24 transition duration-150 ease-in-out sm:text-sm sm:leading-5"
                                                    aria-label="{{ __('Inventory quantity') }}" min="0" autocomplete="off" />
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
    ...
    <x-shopper-modal wire:model="bulkModal" maxWidth="4xl">
        <div>
            <div class="px-4 py-5 sm:p-6 w-full h-auto md:h-125 overflow-y-auto">
                <div>
                    <h2 class="text-lg leading-6 font-medium text-gray-900">
                        {{ __('Create variants from an attribute matrix') }}</h2>
                    <p class="mt-1 text-sm leading-5 text-gray-500">
                        {{ __('Variant name and price. If the price is empty, the price of the product will be applied.') }}
                    </p>
                </div>
                    ...
                    @foreach ($attributes as $attribute)
                        <div class="sm:col-span-3 space-y-5">
                            <div>
                                ...
                                <x-shopper-input.select multiple name="bulkAttributes[{{ $attribute->name }}]" wire:model="bulkAttributes.{{ $attribute->id }}">
                                    @foreach ($attribute->values as $value)
                                        <option value="{{ $value->id }}">{{ $value->key }}</option>
                                    @endforeach
                                </x-shopper-input.select>
                                <span class="text-gray-600">{{ __('Hold ctrl to select multiple attributes') }}</span>
                            </div>
                        </div>
                    @endforeach
                </div>
            ...
            <div class="bg-gray-50 p-4 sm:px-6 sm:flex sm:flex-row-reverse">
                <span class="flex w-full rounded-md shadow-sm sm:ml-3 sm:w-auto">
                    <x-shopper-button type="button" wire:click="generateVariants" wire.loading.attr="disabled">
                        <svg wire:loading wire:target="generateVariants" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                        </svg>
                ...
                <span class="mt-3 flex w-full rounded-md shadow-sm sm:mt-0 sm:w-auto">
                    <x-shopper-default-button type="button" wire:click="closeBulkVariantsModal">
                        {{ __('Cancel') }}
                    </x-shopper-default-button>
                </span>
            </div>
