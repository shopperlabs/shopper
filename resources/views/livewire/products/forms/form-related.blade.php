                    ...
                    <div class="flex py-6 space-x-6 lg:py-0">
                        ...
                        <div class="flex-auto space-y-2">
                            <h3 class="text-secondary-900 dark:text-white">
                                <a href="{{ route('shopper.products.edit', $relatedProduct) }}">{{ $relatedProduct->name }}</a>
                            </h3>
                            <p class="font-medium text-secondary-500 dark:text-secondary-400">{{ $relatedProduct->formattedPrice }}</p>
                            <button wire:click="remove({{ $relatedProduct->id }})" type="button" class="inline-flex text-red-500 text-sm leading-5 focus:outline-none hover:underline">{{ __('shopper::layout.forms.actions.remove') }}</button>
                        </div>
                    </div>
                @empty
                    <div class="text-center lg:col-span-3">
                        <x-heroicon-o-book-open class="mx-auto h-12 w-12 text-secondary-400" />
                        <h3 class="mt-2 text-base font-medium text-secondary-900 dark:text-white">{{ __('shopper::pages/products.related.empty') }}</h3>
                        <p class="mt-1 text-sm text-secondary-500 dark:text-secondary-400">{{ __('shopper::pages/products.related.add_content') }}</p>
                        <div class="mt-6">
                            <x-shopper::buttons.primary type="button" wire:click="$emit('openModal', 'shopper-modals.related-list', {{ json_encode([$product->id, $this->productsIds]) }})">
