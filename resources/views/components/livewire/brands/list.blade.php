<div>
    ...
    @if($brands->total() === 0)
        <div class="mt-6 md:flex md:items-center md:justify-between">
            <div class="flex-1 min-w-0">
                <h2 class="text-2xl font-bold leading-7 text-primary-text sm:text-3xl sm:leading-9 sm:truncate pb-4 border-b border-gray-200">{{ __('Brands') }}</h2>
            </div>
        </div>
        <div class="empty-categories relative w-full flex flex-col md:flex-row items-center justify-end relative py-12 md:py-24">
            ...
            <div class="w-full lg:w-1/2 relative z-90">
                <div class="w-full pl-0 lg:pl-20 lg:pt-20 xl:pt-24">
                    <h3 class="text-primary-text font-medium text-xl mb-4">{{ __('Organize your products into brands') }}</h3>
                    <p class="text-gray-500 text-lg mb-3">{{ __('Create brands to help your customers find products.') }}</p>
                    <a href="{{ route('shopper.brands.create') }}" class="btn btn-primary inline-flex px-6">{{ __('Create brand') }}</a>
                </div>
            </div>
        </div>
    @endif
    ...
    @if($brands->total() > 0)
        <div class="my-6 md:flex md:items-center md:justify-between">
            <div class="flex-1 min-w-0">
                <h2 class="text-2xl font-bold leading-7 text-primary-text sm:text-3xl sm:leading-9 sm:truncate">{{ __('Brands') }}</h2>
            </div>
            <div class="mt-4 flex md:mt-0 md:ml-4">
                <a href="{{ route('shopper.brands.create') }}" class="btn btn-primary">{{ __('Create brand') }}</a>
            </div>
        </div>
            ...
            <div class="px-4 py-4 sm:px-6">
                ...
                <div class="flex rounded-md shadow-sm">
                    <div class="relative flex-grow focus-within:z-10">
                        ...
                        <input id="filter" class="form-input form-input-shopper block w-full rounded-none rounded-l-md pl-10 transition ease-in-out duration-150 sm:text-sm sm:leading-5" placeholder="{{ __('Search categories') }}" />
                    </div>
