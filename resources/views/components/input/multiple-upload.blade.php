    ...
    @if(collect($files)->isNotEmpty())
        <ul class="grid grid-rows-2 grid-cols-7 gap-4">
            @foreach($files as $file)
                <li class="relative group @if($loop->first) row-span-2 col-span-3 h-44 @else col-span-1 h-20 @endif border border-gray-300 dark:border-gray-700 rounded-md overflow-hidden">
