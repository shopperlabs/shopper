<div>
    ...
    @if($categories->total() > 0)
        <div class="my-6 md:flex md:items-center md:justify-between">
            <div class="flex-1 min-w-0">
                <h2 class="text-2xl font-bold leading-7 text-primary-text sm:text-3xl sm:leading-9 sm:truncate">{{ __('Categories') }}</h2>
            </div>
            <div class="mt-4 flex md:mt-0 md:ml-4">
                <a href="{{ route('shopper.categories.create') }}" class="btn btn-primary">{{ __('Create category') }}</a>
            </div>
        </div>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-md">
            ...
            <div class="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
                <div class="flex-1 flex justify-between sm:hidden">
                    {{ $categories->links('shopper::components.wire-mobile-pagination-links') }}
                </div>
