    ...
    @if(collect($files)->isNotEmpty())
        <ul class="grid grid-rows-2 grid-cols-7 gap-4">
            @foreach($files as $file)
                <li @class([
                    'relative group border border-gray-300 dark:border-gray-700 rounded-md overflow-hidden',
            ...
            @foreach($images as $image)
                <div class="relative flex flex-col items-center overflow-hidden h-24 text-center bg-gray-100 border rounded-md select-none dark:bg-gray-700 dark:border-gray-700">
                    <button class="absolute top-0 right-0 z-50 p-1 bg-white rounded-bl focus:outline-none dark:bg-gray-800" type="button" wire:click="removeMedia({{ $image->id }})">
                        <x-untitledui-trash-03 class="w-5 h-5 text-gray-500 dark:text-gray-400" />
                    </button>
                    ...
                    <div wire:loading.class.remove="hidden" wire:target="removeMedia({{ $image->id }})" class="absolute h-full w-full flex items-center justify-center text-center opacity-100 focus-within:opacity-100 inset-0 z-50 bg-gray-800 bg-opacity-75">
