<x-shopper::container>
    ...
    <section aria-labelledby="similar_products_heading">
        <x-shopper::card class="mt-5 p-4 sm:p-5">
            <div role="list" class="text-sm font-medium text-gray-500 divide-y divide-gray-200 dark:text-gray-400 dark:divide-gray-700 lg:grid lg:grid-cols-3 lg:gap-x-8 lg:gap-y-5 lg:divide-none" wire:poll.visible.30s>
                @forelse($relatedProducts as $relatedProduct)
                    <div class="flex py-6 space-x-6 lg:py-0">
                        <img src="{{ $relatedProduct->getFirstMediaUrl(config('shopper.core.storage.collection_name')) }}" alt="{{ $relatedProduct->name }}" class="flex-none w-20 h-20 bg-gray-100 dark:bg-gray-900 rounded-md object-center object-cover">
