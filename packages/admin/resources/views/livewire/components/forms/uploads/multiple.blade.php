<div class="relative" x-data="{ focused: false }">
    <div
        x-ref="dragContainer"
        :class="focused ? 'bg-gray-100/50 dark:bg-gray-700/50 ' : 'border-gray-300 dark:border-gray-700 bg-transparent'"
        class="relative bg-gray-50 dark:bg-gray-800 flex-1 w-full px-6 py-10 group flex justify-center border-2 border-gray-300 border-dashed dark:border-gray-700 hover:bg-gray-100/50 dark:hover:bg-gray-700/50 rounded-lg cursor-pointer transition duration-150 ease-in-out">
        <div class="space-y-1 text-center">
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                <span class="inline-flex items-center px-2.5 py-1.5 border border-gray-300 shadow-sm text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50 dark:bg-gray-800 dark:border-gray-700 dark:text-white focus:outline-none transition duration-150 ease-in-out">
    ...
    @if(collect($files)->isNotEmpty())
        <ul class="mt-4 grid grid-cols-6 gap-4">
            @foreach($files as $file)
                <li class="relative group col-span-1 h-20 border border-gray-300 dark:border-gray-700 rounded-md overflow-hidden">
