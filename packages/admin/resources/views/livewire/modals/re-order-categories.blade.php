                ...
                <div wire:sortable.item="{{ $category->id }}" wire:key="category-{{ $category->id }}" class="pb-5 cursor-move">
                    <div class="p-2 bg-gray-100 dark:bg-gray-700 flex items-center justify-center rounded-md">
                        <div wire:sortable.handle class="flex flex-1 items-center space-x-3">
                            <x-untitledui-grid class="w-5 h-5 text-gray-400 dark:text-gray-500" />
                            <div class="flex items-center space-x-3">
                                <div class="shrink-0 w-2.5 h-2.5 rounded-full {{ $category->is_enabled ? 'bg-green-600': 'bg-gray-400 dark:bg-gray-900' }}"></div>
                            ...
                            @foreach($category->childs as $child)
                                <li wire:key="sub-category-{{ $child->id }}" wire:sortable-group.item="{{ $child->id }}" class="-mx-1">
                                    <div class="flex flex-1 items-center space-x-3 py-1 cursor-move">
                                        ...
                                        <span wire:sortable.handle class="text-sm leading-5 text-gray-500 dark:text-gray-400">
                                            {{ $child->name }}
                                        </span>
                                    </div>
                                </li>
